...
const MovieDetails = () => {
    ...
    const {data} = useAxios<Results>([
      ...
      // `${URL}/movie/${movieInfo[0]?.id}/videos?&api_key=${API_KEY}&language=en-US&page=1`,
      ...
      <header id="header" className="w-full h-auto pt-[70px] flex-wrap flex items-center justify-evenly relative overflow-hidden">
        ...
        <div className="w-full z-30 max-w-[800px] h-auto relative gap-[20px] flex flex-col items-start justify-end p-[30px] pb-[50px]">
          ...
          <div className="flex items-center justify-center gap-[10px]">
            <span className="text-white text-[.85rem]"><i className="text-[#6c49f5]">Release Date: </i>{movieInfo[0]?.release_date}</span>
            <span className="text-white text-[.85rem]"><i className="text-[#6c49f5]">Type:</i> {movieInfo[0]?.media_type}</span>
          </div>
        ...
        <img className="w-full max-w-[400px] md:top-[70px] h-full md:absolute md:z-10 md:max-w-full top-0 left-0 object-cover" src={IMG_URL + movieInfo[0]?.poster_path} alt={movieInfo[0]?.poster_path} />
      </header>
